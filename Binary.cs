...
namespace EA.Core.Encryption
{
    ...
    public class Binary
    {
        ...
        private string Abbreviate(string orig)
        {
            // DB Specific
        ...
        private string Deabbreviate(string abbr)
        {
        ...
        private byte[] EncryptBytes(byte[] clearData)
        {
        ...
        private byte[] DecryptBytes(byte[] cipherData)
        {
        ...
        /// <returns></returns>
        public string CompactGuid(string guid)
        {
        ...
        /// <param name="guid">The GUID.</param>
        /// <returns></returns>
        /// <change>Oliver Fels, 2009/10/13 If guid is already expanded do nothing instead of throwing an error</change>
        public string ExpandGuid(string guid)
        {
            StringBuilder sb = new StringBuilder();

            if (guid.Contains("{") || guid.Contains("-") || guid.Contains("}") || guid.Length != 32)
            {
                //already in expanded form? return it
                return guid;
            }
